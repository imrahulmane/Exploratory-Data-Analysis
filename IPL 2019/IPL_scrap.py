...
csv_writer.writerow(['team_1','team_2','team_1_score', 'team_2_score','venue', 'winner'])


for whole in soup.find_all('div',class_=re.compile('js-match match-list__item result \w+')):
    count = 1
    for team in whole.find_all('p',class_='result__team-name'):
        if count == 1:
            team_1 = team.text
        elif count == 2:
            team_2 = team.text
    ...
    win_count = 1
    for win_score in whole.find_all('span', class_='result__score'):
        if win_count == 1:
    ...
    winner = whole.find('p', class_='result__outcome u-hide-phablet') #winner
    winner = winner.text
